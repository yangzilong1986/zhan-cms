...
function createGoFirstPage(p_pn,p_url_para,p_searchType,formname){
	...
	var temp;
	if(p_pn<2){
		temp="class=page_button_disabled disabled=true ";
	}
	else{
		temp=" class=list_button_active";
	}
	m_str="<td class='list_form_button_td'><input type='submit' name='b1' "+temp+"  value=' 首页 '   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+"1\")'>";
...
function createGoPrevPage(p_pn,p_url_para,p_searchType,formname){
	...
	if(p_pn<2){
		...
		m_pn=p_pn-1;
			...
			m_str="<td class='list_form_button_td'><input type='submit' name='b2' "+temp+"  value='上一页'   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'>";
...
function createGoNextPage(p_pn,p_pageCount,p_url_para,p_searchType,formname){
	...
	if(p_pn>=p_pageCount){
		...
		//m_str="<a href='"+p_url_para+m_pn+"'>"+m_str+"</a>";
		//m_str="<a href='javascript:flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'>"+m_str+"</a>";
		m_str="<td class='list_form_button_td'><input type='submit' name='b3' "+temp+" class='page_button_active'  value='下一页'   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'>";
...
function createGoLastPage(p_pn,p_pageCount,p_url_para,p_searchType,formname){
	...
	m_pn=p_pageCount;
	m_str="<td class='list_form_button_td'><input type='submit' name='b4' "+temp+" class='page_button_active'  value=' 尾页 '   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'>";
	return m_str;
}
//************************************************内部函数：翻页执行
