...
function createGoFirstPage(p_pn,p_url_para,p_searchType,formname){
	...
	m_str="<td class='list_form_button_td'><input type='button'  "+temp+"  value=' 首页 '   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+"1\")'></td>";
...
function createGoPrevPage(p_pn,p_url_para,p_searchType,formname){
	...
	if(p_pn<2){
		...
		m_pn=p_pn-1;
			...
			m_str="<td class='list_form_button_td'><input type='button'  "+temp+"  value='上一页'   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'></td>";
...
function createGoNextPage(p_pn,p_pageCount,p_url_para,p_searchType,formname){
	...
	if(p_pn>=p_pageCount){
		...
		//m_str="<a href='"+p_url_para+m_pn+"'>"+m_str+"</a>";
		//m_str="<a href='javascript:flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'>"+m_str+"</a>";
		m_str="<td class='list_form_button_td'><input type='button'  "+temp+"  value='下一页'   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'></td>";
...
function createGoLastPage(p_pn,p_pageCount,p_url_para,p_searchType,formname){
	...
	m_pn=p_pageCount;
	m_str="<td class='list_form_button_td'><input type='button'  "+temp+" value=' 尾页 '   onClick='flippagesubmit(\""+formname+"\",\""+p_url_para+m_pn+"\")'></td>";
	return m_str;
}
//************************************************内部函数：翻页执行
