...
<form name="form1" method='post' action="fl_info2.jsp">
...
<a href="#" onClick="return refselect_FCCLASS(opener.winform.CMT3,'<%=DBUtil.fromDB(crs.getString("REASON"))%>')"><%=level.getEnumItemName("LoanCat1",crs.getString("FCCLASS"))%></a>
</td>
<td class='reference_button_tbl_td'>
<a href="#" onClick="return refselect_FCCLASS(opener.winform.CMT3,'<%=DBUtil.fromDB(crs.getString("REASON"))%>')"><%=DBUtil.fromDB(crs.getString("REASON"))%></a>
</td>
 </tr>
 <%
}
...
<td class='reference_button_tbl_td'>
<input class='reference_button_active' type='button' name='submit3' value=' 过 滤 ' onclick="if ( filter.style.visibility == 'hidden' ) { filter.style.visibility ='';} else { filter.style.visibility = 'hidden';}"></td>
<script language="javascript">
    		createFlipPage(<%=pn%>,<%=ps%>,<%=rows%>,"fl_info2.jsp?pn=","form1");
</script>
<td>
