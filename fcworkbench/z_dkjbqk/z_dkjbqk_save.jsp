...
String CMT2=null;
  ...
  String sql="";
  String query_sql="select *from FCCMT where FCNO='"+FCNO+"' and FCCMTTYPE=2";
  CachedRowSet crs=manager.getRs(query_sql);
  if(crs.next()){
	sql="update FCCMT set cmt1='"+DBUtil.toDB(CMT1)+"',cmt2='"+DBUtil.toDB(CMT2)+"',cmt3='"+DBUtil.toDB(CMT3)+"',cmt4='"+DBUtil.toDB(CMT4)+"'"+
		" where FCNO='"+FCNO+"' and FCCMTTYPE=2";
  }
  else{
	sql="insert into FCCMT(FCNO,FCCMTTYPE,CMT1,CMT2,CMT3,CMT4) values('"+FCNO+"',2,'"+DBUtil.toDB(CMT1)+"','"+DBUtil.toDB(CMT2)+"','"+DBUtil.toDB(CMT3)+"','"+DBUtil.toDB(CMT4)+"')";
  }
  ...
  if(temp){
	...
	%>
	<jsp:forward page="/fcworkbench/z_dkjbqk/z_dkjbqk.jsp"/>
	<%
  }
  else{
