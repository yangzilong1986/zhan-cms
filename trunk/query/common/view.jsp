...
var xmlhttp;
    ...
    window.close();
		...
		createXMLHttpRequest();
 		var url="servlet/UserServlet?brhid="+id;
	...
	if(xmlhttp.readyState==4){
		if(xmlhttp.status==200){
 		clearElement('CLIENTMGR');
 		parsElement('CLIENTMGR');
 		clearElement('FIRSTRESP');
 		parsElement('FIRSTRESP');
		}}}
		
function clearElement(id)
{
var header=opener.window.document.getElementById(id);
for(var i=header.options.length;i>=0;i--){
	header.remove(i);
}
}
function parsElement(id){
...
var values=results.getElementsByTagName("value");
for(var i=0;i<names.length;i++){
var op=opener.window.document.createElement('option');
op.text=names[i].childNodes[0].nodeValue;
op.value=values[i].childNodes[0].nodeValue;
header.add(op);
op=null;
property=null;
name=null;
...
<table class='reference_tbl'>
	...
	<tr class='reference_tbl_content_tr'>
		<td class='reference_button_tbl_td'><a href="javascript:;"
			onClick="return refselect_brhid1(opener.form1.BRHID,'<%=DBUtil.fromDB(crs.getString("brhid"))%>/<%=DBUtil.fromDB(crs.getString("lname"))%>')"><%=crs.getString("brhid")%> </a></td>
		<td class='reference_button_tbl_td'><a href="javascript:;"
			onClick="return refselect_brhid1(opener.form1.BRHID,'<%=DBUtil.fromDB(crs.getString("brhid"))%>/<%=DBUtil.fromDB(crs.getString("lname"))%>')"><%=DBUtil.fromDB(crs.getString("lname"))%> </a></td>
	</tr>
	<%
	}
	%>
