package zt.cms.bm.bill.fieldformat;
...
public class BillDisFieldFormat extends FieldFormat
{
...
//            return body;
//        } else {
        System.out.println("["+values.getString("BMNO")+"]"+ctx.getUrl(actionurl));

            String body = "<a class=\"list_edit_href\" href=\"#\" onclick=\"window.open('" +
                ctx.getUrl(actionurl) + "?" + value +"&BMNO="+values.getString("BMNO").trim()+
                "','FI"+fi.getInstanceid()+"','height=500,width=600,toolbar=no,scrollbars=yes,resizable=yes');\">" + "贴现" +
                "</a>";
