...
//    String FC3 = "";//联社主任认定结果
    String LMFCCLASS = "";//上月清分结果      lj added in 2007-04-25
    String BMTYPE = "";//业务类型代码
    String BILLNO = "";//票据台帐代码
    String BADREASON = "";//贷款形成不良原因  lj added in 2007-04-27
    String LOANWAY = "";  //贷款投向         lj added in 2007-04-27
...
//        FC3 = crs.getString("FC3");//联社主任认定结果
        LMFCCLASS = (crs.getString("LMFCCLASS") == null) ? "0" : crs.getString("LMFCCLASS");//上月清分结果 lj added in 2007-04-25
        BMTYPE = crs.getString("BMTYPE");//业务类型代码
        BILLNO = crs.getString("BILLNO");//票据台帐代码
        BADREASON = crs.getString("BADREASON");//贷款形成不良原因                      lj added in 2007-04-27
        LOANWAY = crs.getString("LOANWAY");    //贷款投向                              lj added in 2007-04-27
    ...
    String CMT3 = "";
    String CMT4 = "";
//FC2=request.getParameter("FC2");

    CMT3 = request.getParameter("CMT3");
    if (BADREASON == null || BADREASON.equals("")) BADREASON = request.getParameter("BADREASON");  //lj added in 2007-05-14
    if (FC2 == null || FC2.equals("")) FC2 = request.getParameter("FC2");                          //lj added in 2007-05-16
    ...
    String readonly = "";
    String submit = "";
    //lj deleted Hyperlink in 2007-04-15 for delete the flinfo2.jsp
...
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap><input type="text" readonly name="FCAUTO"
        ...
        <input type="hidden" name="FCAUTOINT" value="<%=FCAUTO%>">
        <input type="hidden" name="LMFCCLASS" value="<%=LMFCCLASS%>"></td>
...
<tr class='page_form_tr'>
    ...
    <td class="page_form_td"
        nowrap><%=FCSTATUS.equals(String.valueOf(EnumValue.FCStatus_SanJiShenPi)) ? level.levelHereExt("FC2", "LoanCat1", FC2, "") : level.levelHereExt1("FC2", "LoanCat1", FC2, "")%>
    </td>
</tr>
<tr class='page_form_tr'>
...
<tr class='page_form_tr'>
    <td class="page_form_td" nowrap>
        <a href="#" class="list_edit_href" onClick="deptrefer_click('CMT3','FC2')">信贷部门主任清分*<br>意见</a>
    </td>
...
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap colspan="3"><textarea name="CMT4" readonly wrap="PHYSICAL" cols="57" rows="4"
...
<tr class='page_form_tr'>
    <td class='page_form_td' colspan='6'>
...
function Regvalid() {
    var FC2 = form1.FC2;
    var FC2VAL = FC2.value;
    var CMT3 = form1.CMT3;
    var CMT3VAL = CMT3.value.Trim();
    ...
    if (FC2VAL < 3 && document.all.badreasontitle.innerHTML.indexOf("*") > 0) {
        document.all.badreasontitle.innerHTML = document.all.badreasontitle.innerHTML.split("*")[0];
    } else if (FC2VAL > 2 && document.all.badreasontitle.innerHTML.indexOf("*") < 0)
        document.all.badreasontitle.innerHTML += "*";


    if (FC2VAL > 2 && BADREASONVAL.length == 0) {   //lj added in 2007-04-28
        alert("请选择贷款形成不良原因！");
        BADREASON.focus();
        return false;
    }
        ...
        LOANWAY.focus();
        return false;
    } else if (FC2VAL.length == 0) {
        alert("请输入正确的信贷部门主任认定结果！");
        FC2.focus();
        return false;
    }
    else if (CMT3VAL.length == 0 || CMT3VAL.length > 300) {
        alert("请输入正确的信贷部门主任清分意见！");
        CMT3.focus();
        return false;
    }
    else {
        <%--if (<%=FC2%>!= null && <%=FC2%>!="" && FC2VAL != <%=FC2%>) {--%>
            <%--if(!confirm("您确认更改分类结果后已输入正确的分类意见了吗?"))return false;--%>
        <%--}--%>
        if(!confirm("您的清分结果是'"+FC2.options[FC2.selectedIndex].text+"',请确认"))return false;
        form1.action = "two_save.jsp?FCNO=<%=FCNO%>&FCSTATUS=<%=FCSTATUS%>&FCTYPE=<%=FCTYPE%>&BMNO=<%=BMNO%>&BMTYPE=<%=BMTYPE%>&BILLNO=<%=BILLNO%>&CLIENTNO=<%=CLIENTNO%>&CREATEDATE=<%=CREATEDATE%>";  //lj added CLIENTNO,CREATEDATE in 2007-05-28
        form1.submit();
    }

}

function deptrefer_click(cmt, chaFcclass) { //lj added param cmt,chaFcclass in 2007-04-15 for delete the flinfo2.jsp
