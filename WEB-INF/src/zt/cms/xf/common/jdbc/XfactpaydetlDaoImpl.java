...
calls to this DAO, otherwise a new Connection will be allocated for each operation.
	 ...
	 */
	protected final String SQL_SELECT = "SELECT JOURNALNO, CONTRACTNO, CPANO, PAYDATE, PAYSTATUS, PAYAMT, PAYACT, RECVACT, RECVBANKID, RECVBANKNO, RECVBANKNAME, OPERATORID, OPERATEDATE, CHECKERID, CHECKDATE FROM " + getTableName() + "";
	 ...
	 */
	protected final String SQL_INSERT = "INSERT INTO " + getTableName() + " ( JOURNALNO, CONTRACTNO, CPANO, PAYDATE, PAYSTATUS, PAYAMT, PAYACT, RECVACT, RECVBANKID, RECVBANKNO, RECVBANKNAME, OPERATORID, OPERATEDATE, CHECKERID, CHECKDATE ) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";

	/** 
	 * SQL UPDATE statement for this table
	 */
	protected final String SQL_UPDATE = "UPDATE " + getTableName() + " SET JOURNALNO = ?, CONTRACTNO = ?, CPANO = ?, PAYDATE = ?, PAYSTATUS = ?, PAYAMT = ?, PAYACT = ?, RECVACT = ?, RECVBANKID = ?, RECVBANKNO = ?, RECVBANKNAME = ?, OPERATORID = ?, OPERATEDATE = ?, CHECKERID = ?, CHECKDATE = ? WHERE JOURNALNO = ?";
	...
	public void update(XfactpaydetlPk pk, Xfactpaydetl dto) throws XfactpaydetlDaoException
	{
		...
		try {
			...
			stmt.setString( index++, dto.getCheckerid() );
			stmt.setTimestamp(index++, dto.getCheckdate()==null ? null : new java.sql.Timestamp( dto.getCheckdate().getTime() ) );
			stmt.setString( 16, pk.getJournalno() );
			int rows = stmt.executeUpdate();
			reset(dto);
			long t2 = System.currentTimeMillis();
			System.out.println( rows + " rows affected (" + (t2-t1) + " ms)" );
