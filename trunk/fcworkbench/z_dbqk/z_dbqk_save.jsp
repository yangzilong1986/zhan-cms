...
String CMT2=null;
  ...
  String sql="";
  String query_sql="select *from FCCMT where FCNO='"+FCNO+"' and FCCMTTYPE=4";
  ...
  if(crs.next()){
	sql="update FCCMT set cmt1='"+DBUtil.toDB(CMT1)+"',cmt2='"+DBUtil.toDB(CMT2)+"',cmt3='"+DBUtil.toDB(CMT3)+"',cmt4='"+DBUtil.toDB(CMT4)+"',cmt5='"+DBUtil.toDB(CMT5)+"'"+
		" where FCNO='"+FCNO+"' and FCCMTTYPE=4";
  }
  else{
	sql="insert into FCCMT(FCNO,FCCMTTYPE,CMT1,CMT2,CMT3,CMT4,CMT5) values('"+FCNO+"',4,'"+DBUtil.toDB(CMT1)+"','"+DBUtil.toDB(CMT2)+"','"+DBUtil.toDB(CMT3)+"','"+DBUtil.toDB(CMT4)+"','"+DBUtil.toDB(CMT5)+"')";
  }
  ...
  if(temp){
	...
	%>
	<jsp:forward page="/fcworkbench/z_dbqk/z_dbqk.jsp"/>
	<%
  }
  else{
