...
function checkSubmit(){
	...
	var constarea   =document.all.CONSTAREA;
	...
	if(parseInt(constarea.value,10)<=0){
		alert("建筑面积必须大于0！");
		constarea.focus();
		return false;
	}
	if(parseInt(area.value,10)<=0){
		alert("面积必须大于0！");
		area.focus();
		return false;
	}
	if(parseInt(firsttermrate.value,10)<=0){
		alert("首期实付款占比必须大于0！");
		firsttermrate.focus();
		return false;
	}
	if(parseInt(monthlydue.value,10)<=0){
		alert("月还款本金必须大于0！");
		monthlydue.focus();
		return false;
	}
	if(parseInt(monthlyduetotal.value,10)<=0){
		alert("月还款本息必须大于0！");
