...
Title: 清分操作-信贷员认定
Description: 信贷员认定。
 * @version  $Revision: 1.16 $  $Date: 2007/06/20 08:20:31 $
 * @author liujian
 * <p/>修改：$Author: liuj $
===============================================
--%>
...
//        FC3 = crs.getString("FC3");//联社主任认定结果
    ...
    String CMT3 = "";
    String CMT4 = "";
//FC1=request.getParameter("FC1");
    CMT1 = request.getParameter("CMT1");
    CMT2 = request.getParameter("CMT2");
    if (BADREASON == null || BADREASON.equals("")) BADREASON = request.getParameter("BADREASON");  //lj added in 2007-05-14
    if (LOANWAY == null || LOANWAY.equals("")) LOANWAY = request.getParameter("LOANWAY");          //lj added in 2007-05-14
    if (FC1 == null || FC1.equals("")) FC1 = request.getParameter("FC1");                          //lj added in 2007-05-16
    ...
    String readonly = "";
    String submit = "";
    //lj deleted Hyperlink in 2007-04-15 for delete the flinfo1.jsp
...
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap><%=level.levelHereExt("LOANWAY", "LoanWay", LOANWAY, "")%>
    </td>
</tr>
<!--lj added end-->
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap>
        <%=FCSTATUS.equals(String.valueOf(EnumValue.FCStatus_DengJi)) ? level.levelHereExt("FC1", "LoanCat1", FC1, "") : level.levelHereExt1("FC1", "LoanCat1", FC1, "")%>
    </td>
</tr>
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap><%=level.levelHereExt1("FC2", "LoanCat1", "", "")%>
    </td>
</tr>
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap><%=level.levelHereExt1("FC3", "LoanCat1", "", "")%>
    </td>
</tr>
...
<tr class='page_form_tr'>
    <td class="page_form_td" nowrap>
        <a href="#" class="list_edit_href" onClick="deptrefer_click('CMT1','FC1')">风险评级理由及*<br>初步分类意见</a>
    </td>
    <td class="page_form_td" nowrap colspan="3">
        <textarea name="CMT1" <%=readonly%> wrap="PHYSICAL" cols="57" rows="4"
...
<tr class='page_form_tr'>
    <!--lj added FCType_WeiXingQiYe in 2007-05-15-->
    <td class="page_form_td" nowrap>
        <a href="#" class="list_edit_href"
           onclick="deptrefer_click('CMT2','FC1')"><%=FCTYPE.equals(String.valueOf(EnumValue.FCType_QiYe)) || FCTYPE.equals(String.valueOf(EnumValue.FCType_WeiXingQiYe)) ? "信贷讨论分类意见*" : "客户经理清分意见*"%>
        </a>
    </td>
    <td class="page_form_td" nowrap colspan="3">
        <textarea name="CMT2" <%=readonly%> wrap="PHYSICAL" cols="57" rows="4"
...
<tr class='page_form_tr'>
    ...
    <td class="page_form_td" nowrap colspan="3">
        <textarea name="CMT3" readonly wrap="PHYSICAL" cols="57" rows="4"
...
<tr class='page_form_tr'>
    <td class='page_form_td' colspan='6' align='center'><input type="button" name="CorpCapital" value=" 客户基本信息 "
                                                               class="page_form_button_active"
...
document.focus();
if (form1.FC1.value > 2)document.all.badreasontitle.innerHTML += "*";
function deptrefer_click(cmt, chaFcclass) { //lj added param cmt,chaFcclass in 2007-04-15 for delete the flinfo1.jsp
    var url = "Hyperlink/fl_info.jsp?cmt=" + cmt + "&chaFcclass=" + chaFcclass;
    window.open(url, 0, "height=450,width=470,toolbar=no,scrollbars=yes,status");
}

function Regvalid() {
    var FC1 = form1.FC1;
    var FC1VAL = FC1.value;
    var CMT1 = form1.CMT1;
    var CMT1VAL = CMT1.value.Trim();
    var CMT2 = form1.CMT2;
    var CMT2VAL = CMT2.value.Trim();
    ...
    if (FC1VAL < 3 && document.all.badreasontitle.innerHTML.indexOf("*") > 0) {
        document.all.badreasontitle.innerHTML = document.all.badreasontitle.innerHTML.split("*")[0];
    } else if (FC1VAL > 2 && document.all.badreasontitle.innerHTML.indexOf("*") < 0)
        document.all.badreasontitle.innerHTML += "*";


    if (FC1VAL > 2 && BADREASONVAL.length == 0) {   //lj added in 2007-04-28
        alert("请选择贷款形成不良原因！");
        BADREASON.focus();
        return false;
    }
    ...
    else if (FC1VAL.length == 0) {
        alert("请输入正确的客户经理认定结果！");
        FC1.focus();
        return false;
    }
    else if (CMT1VAL.length == 0 || CMT1VAL.length > 300) {
        alert("请输入正确的风险评级理由及初步分类意见！");
        CMT1.focus();
        return false;
    }
    else if (CMT2VAL.length == 0 || CMT2VAL.length > 300) {
        alert("请输入正确的信贷讨论分类意见！");
        CMT2.focus();
        return false;
    }
    else {
        <%--if (<%=FC1%>!= null && <%=FC1%>!="" && FC1VAL != <%=FC1%>) {--%>
            <%--if(!confirm("请确认您的清分结果及清分意见已变更为?"))return false;--%>
        <%--}--%>
        if(!confirm("您的清分结果是'"+FC1.options[FC1.selectedIndex].text+"',请确认"))return false;
        form1.action = "one_save.jsp?FCNO=<%=FCNO%>&FCSTATUS=<%=FCSTATUS%>&FCTYPE=<%=FCTYPE%>&CLIENTNO=<%=CLIENTNO%>&CREATEDATE=<%=CREATEDATE%>";  //lj added CLIENTNO,CREATEDATE in 2007-05-28
        form1.submit();
    }
...
function newAffair(affairName) {
    ...
    if (affairName == 'CorpPayables') {
        var url = "history_qf/history_qf.jsp?BMNO=<%=BMNO%>&FCNO=<%=FCNO%>&CREATEDATE=<%=CREATEDATE%>&CREATEDATE=<%=CREATEDATE%>";
