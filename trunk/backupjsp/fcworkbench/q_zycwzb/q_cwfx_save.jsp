...
String CMT1=null;
  ...
  String sql="";
  String query_sql="select *from FCCMT where FCNO='"+FCNO+"' and FCCMTTYPE=10";
  CachedRowSet crs=manager.getRs(query_sql);
  if(crs.next()){
	sql="update FCCMT set cmt1='"+DBUtil.toDB(CMT1)+"' where FCNO='"+FCNO+"' and FCCMTTYPE=10";
  }
  else{
	sql="insert into FCCMT(FCNO,FCCMTTYPE,CMT1) values('"+FCNO+"',10,'"+DBUtil.toDB(CMT1)+"')";
  }
  ...
  if(temp){
	...
	%>
	<jsp:forward page="/fcworkbench/q_cwfx/q_zycwzb.jsp"/>
	<%
  }
  else{
	request.setAttribute("mess","添加信息失败");
	%>
	<jsp:forward page="/fcworkbench/q_cwfx/q_cwfx.jsp"/>
	<%
  }
%>