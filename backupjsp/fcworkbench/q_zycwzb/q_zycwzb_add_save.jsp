...
String AMT1=null;
 ...
 String AMT9=null;
  ...
  if(request.getParameter("CMT1")==null){
	...
	String query_sql="select * from FCMCMT where FCNO='"+FCNO+"' and FCCMTTYPE=10 and DT="+DBUtil.toSqlDate(DT)+"";
	CachedRowSet crs=manager.getRs(query_sql);
	if(crs.next()){
		request.setAttribute("mess","日期:"+DT.substring(0,6)+"已存在");
	%>
	<jsp:forward page="/fcworkbench/q_zycwzb/q_zycwzb_add.jsp"/>
	<%
	}
  String sql="insert into FCMCMT(FCNO,FCCMTTYPE,DT,AMT1,AMT2,AMT3,AMT4,AMT5,AMT6,AMT7,AMT8,AMT9,AMT10) "+
  "values('"+FCNO+"',10,"+DBUtil.toSqlDate(DT)+","+AMT1+","+AMT2+","+AMT3+","+AMT4+","+AMT5+","+AMT6+","+AMT7+","+AMT8+","+AMT9+","+AMT10+")";
