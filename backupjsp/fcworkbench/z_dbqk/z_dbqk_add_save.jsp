...
String CMT2=null;
  ...
  if(request.getParameter("CMT4")==null){
	...
	String query_sql="select * from FCMCMT where FCNO='"+FCNO+"' and FCCMTTYPE=4 and DT="+DBUtil. toSqlDate(DT)+"";
	CachedRowSet crs=manager.getRs(query_sql);
	if(crs.next()){
		request.setAttribute("mess","日期:"+DBUtil. toSqlDate(DT)+"已存在");
	%>
	<jsp:forward page="/fcworkbench/z_dbqk/z_dbqk_add.jsp"/>
	<%
	}
    String sql="insert into FCMCMT(FCNO,FCCMTTYPE,DT,CMT1,CMT2,CMT3,CMT4) values('"+FCNO+"',4,"+DBUtil.toSqlDate(DT)+",'"+DBUtil.toDB(CMT1)+"','"+DBUtil.toDB(CMT2)+"','"+DBUtil.toDB(CMT3)+"','"+DBUtil.toDB(CMT4)+"')";
