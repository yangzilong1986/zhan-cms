...
/****************************************************
 *
 * 抵债资产审批(第一级，第二级，第三级公用)
 *
 ****************************************************/

public class BmDecPdAssetPage extends FormActions {
  ...
  private String strBmActType = "0"; //审批级别
  ...
  public int beforeInsert(SessionContext ctx, DatabaseConnection conn, FormInstance instance, ErrorMessages msgs,
    ...
    String strAMT2 = null; //决策利息

    strAMT2=DBUtil.getCellValue(conn, "BMAPPINACTIVELOAN", "APPAMT2", "BMNO='" + strBmNo+"'");
    ...
    if (strBmActType.equals("2")) { //如果为第三级审批则获得登记的信息
      ...
      //instance.setValue("FISRTRESPPCT",Integer.parseInt(strFirstRespPct));//上一业务步骤的第一责任人比例
      instance.setValue("FISRTRESPPCT", strFirstRespPct); //上一业务步骤的第一责任人比例FISRTRESPPCT
    }

    instance.setValue("CURNO", strCurNo); //上一业务步骤的货币代码
    instance.setValue("AMT", strAMT); //上一业务步骤的决策金额
    instance.setValue("AMT2", strAMT2); //上一业务步骤的抵顶利息，只有不良核销有
  ...
  public int preEdit(SessionContext ctx, DatabaseConnection conn, FormInstance instance,
                     ErrorMessages msgs, EventManager manager, SqlAssistor assistor) {
    String strIsFirst = ctx.getParameter("IFRESPLOAN").trim();
    if (strIsFirst.equals("1")) {
      String strClientMgr = ctx.getParameter("FIRSTRESP").trim();
      String strWhere = " loginname='" + strClientMgr + "' and usertype<>'3'";
  ...
  public int postInsertOk(SessionContext ctx, DatabaseConnection conn, FormInstance instance, ErrorMessages msgs,
    ...
    try {
      ...
      if (strOthers.equals("1")) {
        iflag = BMTrans.compltTrans(strBmNo, Integer.parseInt(strBmTransNo), strUserName); //完成审批
      }
      if (iflag >= 0) {
        String msg = "信息保存成功！";
