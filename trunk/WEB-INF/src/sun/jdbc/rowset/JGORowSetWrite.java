    ...
    implements RowSetWriter, Serializable
{
    ...
    public JGORowSetWrite()
    {
    }

    private void buildKeyDesc(CachedRowSet cachedrowset)
        throws SQLException
    {
    ...
    private String buildTableName(DatabaseMetaData databasemetadata, String s, String s1)
        throws SQLException
    {
        return s1;
        /*
    ...
    private String buildWhereClause(String s, ResultSet resultset)
        throws SQLException
    {
        s = "WHERE ";
    ...
    private boolean deleteOriginalRow(CachedRowSet cachedrowset)
        throws SQLException
    {
        int k = 0;
    ...
    private void initSQLStatements(CachedRowSet cachedrowset)
        throws SQLException
    {
    ...
    private boolean insertNewRow(CachedRowSet cachedrowset, PreparedStatement preparedstatement)
        throws SQLException
    {
        boolean flag = false;
        try
        {
            for(int i = 1; i <= cachedrowset.getMetaData().getColumnCount(); i++)
            {
                ...
                if(obj != null)
                {
                    ...
                    else if(cachedrowset.getMetaData().getColumnTypeName(i).toUpperCase().indexOf("CHAR") >= 0)
                    {
                      preparedstatement.setString(i,cachedrowset.getString(i));
                    }
                    else
    ...
    public void setReader(RowSetReaderImpl rowsetreaderimpl)
        throws SQLException
    {
        ...
        throws SQLException
    {
        ...
        while(cachedrowset.next())
            if((!cachedrowset.rowDeleted() || !cachedrowset.rowInserted()) && (cachedrowset.rowDeleted() ? (flag = deleteOriginalRow(cachedrowset)) : cachedrowset.rowInserted() ? (flag = insertNewRow(cachedrowset, preparedstatement)) : cachedrowset.rowUpdated() && (flag = updateOriginalRow(cachedrowset))))
                break;
        preparedstatement.close();
