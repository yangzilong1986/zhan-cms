...
<%
    ...
    String sql = "select * from FCREVIEWLIMIT where BRHID='" + brhid + "' ";


    BigDecimal LIMITAMT = null;
    BigDecimal FCTypeAMT = null;

    try {
        CachedRowSet crs = manager.getRs(sql);
        if (crs.next()) {
            LIMITAMT = crs.getBigDecimal("LIMITAMT");
            FCTypeAMT = crs.getBigDecimal("FCTypeAMT");
        }
    } catch (Exception e) {
        e.printStackTrace();
    }
...
<form id='winform' name="form1" method="post" action="Ljd_update_save.jsp" onsubmit="return Regvalid(this)">
                    ...
                    <table class='page_form_table' id='page_form_table' width="300" height="20">
                        ...
                        <tr class='page_form_tr'>
                            ...
                            <td class="page_form_td" nowrap>
                                <input type="text" name="brhid" value="<%=brhid%>"
                                       class="page_form_text" size="25" readonly>
                        ...
                        <tr class='page_form_tr'>
                            ...
                            <td class="page_form_td" nowrap>
                                <input type='text' name='brhjb'
                                       value="<%=SCBranch.getBrhlevel(brhid)==null?"":level.getEnumItemName("BrhLevel",SCBranch.getBrhlevel(brhid))%>"
                                       class="page_form_text" size="25" readonly>
                            </td>
                        </tr>
                        ...
                        <tr class='page_form_tr'>
                            ...
                            <td class="page_form_td" nowrap>
                                <input type='text' name='brhname'
                                       value="<%=SCBranch.getLName(brhid)==null?"":SCBranch.getLName(brhid)%>"
                                       class="page_form_text" size="25" readonly>
                            </td>
                        </tr>
                        <tr class='page_form_tr'>
                            ...
                            <td class="page_form_td" nowrap><input type='text' name='FCTypeAMT'
                                                                   value='<%=FCTypeAMT==null?"":df.format(FCTypeAMT)%>'
                                                                   class="page_form_text" size="25"
                                                                   maxlength='14'></td>
                        </tr>
                        <tr class='page_form_tr'>
                            <td class="page_form_td" nowrap>临界点额度*</td>
                            <td class="page_form_td" nowrap><input type='text' name='LIMITAMT'
                                                                   value='<%=LIMITAMT==null?"":df.format(LIMITAMT)%>'
                                                                   class="page_form_text" size="25"
                                                                   maxlength='14'></td>
                        </tr>
                        <tr class='page_form_tr'>
...
<script language="javascript" type="text/javascript">
    ...
    function deptrefer_click() {
        var url = "view.jsp";
        var vArray;
        document.all.referValue.value = window.open(url, 0, "height=350,width=460,toolbar=no,scrollbars=yes");
        return document.all.referValue.value != "undefined";
    }
