...
function Passwd_Change(){
	var Instance_ID=document.all("Plat_Form_Request_Instance_ID").value;
	var Event_Value=document.all("Plat_Form_Request_Event_Value").value;
	var url="/templates/defaultform.jsp?Plat_Form_Request_Instance_ID="+Instance_ID+"&Plat_Form_Request_Event_ID=2&Plat_Form_Request_Event_Value="+Event_Value+"&CHGTYPE=PASSWDCHG";
	document.all("winform").action=url;
	document.all("winform").submit();
}
...
function pwdenter(me)
{
	if(event.keyCode == 13)
	{
		me.onchange();
		event.returnValue=false;
		document.all.LOANAMT.focus();
	}	
}