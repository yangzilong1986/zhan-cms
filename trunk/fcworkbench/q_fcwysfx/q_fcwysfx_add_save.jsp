...
String CMT1=null;
  ...
  String query_sql="select * from FCMCMT where FCNO='"+FCNO+"' and FCCMTTYPE=8 and DT="+DBUtil.toSqlDate(DT)+"";
	CachedRowSet crs=manager.getRs(query_sql);
	if(crs.next()){
		request.setAttribute("mess","日期:"+DT.substring(0,6)+"已存在");
	%>
	<jsp:forward page="/fcworkbench/q_fcwysfx/q_fcwysfx_add.jsp"/>
	<%
	}

  String sql="insert into FCMCMT(FCNO,FCCMTTYPE,DT,CMT1) "+
  "values('"+FCNO+"',8,"+DBUtil. toSqlDate(DT)+",'"+DBUtil.toDB(CMT1)+"')";
